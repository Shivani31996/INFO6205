...
class ParSort {
    ...
    public static void sort(int[] array, int from, int to, ForkJoinPool forkPool) {
        if (to - from < cutoff) Arrays.sort(array, from, to);
            ...
            CompletableFuture<int[]> parsort1 = parsort(array, from, from + (to - from) / 2,forkPool); // TO IMPLEMENT
            CompletableFuture<int[]> parsort2 = parsort(array, from + (to - from) / 2, to,forkPool); // TO IMPLEMENT
    ...
    private static CompletableFuture<int[]> parsort(int[] array, int from, int to, ForkJoinPool forkPool) {
        return CompletableFuture.supplyAsync(
                () -> {
                    ...
                    sort(result, 0, to - from, forkPool);
                    return result;
                }, forkPool
        );
    }
